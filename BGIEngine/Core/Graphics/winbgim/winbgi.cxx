...
// Internal structures and routines
...
{
    WNDCLASSEXA wcex;                        // The main window class that we create
    ...
    wcex.cbWndExtra = 0;
    wcex.hInstance = BGI__hInstance;

    if (game_icon.length() != 0)
    {
        wcex.hIcon = (HICON) LoadImageA(NULL,game_icon.c_str(),IMAGE_ICON,0,
                                          0,LR_LOADFROMFILE|LR_DEFAULTSIZE|LR_SHARED) ;
    }
    ...
    if (game_icon_small.length() != 0)
    {
        wcex.hIconSm = (HICON) LoadImageA(NULL,game_icon_small.c_str(),IMAGE_ICON,0,
                                          0,LR_LOADFROMFILE|LR_DEFAULTSIZE|LR_SHARED) ;
    }
...
( int width, int height, const char* title, int left, int top, bool dbflag , bool closeflag)
{
    ...
    if ( hThread == NULL )
    {
        showerrorbox((char*)"Thread Not Initialized");
        delete pWndData;
        return -1;
    }
